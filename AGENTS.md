...
- Regeneration has two interchangeable translators in sibling folders under `tools/translator/`, both emitting the same answerable-only, pre-deduped output files: `pnpm translate-etlex` (= `tsx tools/translator/translator-etlex/index.ts`; CSV merge with the Thai lexicon → both public files + `translator-etlex/missing.txt`), or `pnpm translate-ai` (= `tsx tools/translator/translator-gemini/gemini.ts`; Gemini via `@google/genai`, needs `GEMINI_API_KEY` in the process env — tsx does not load `.env.local`: batches of 20 words with POS-specific prompts — verb vs noun must translate differently — journals every finished batch to `translator-gemini/ai-cache.json` and resumes from it on re-run (delete entries there to force a re-translate), `--limit=N` implies `--dry-run`, paced to the model's RPM quota via `GEMINI_RPM`, and output lands in `translator-gemini/` — copy it to `public/` after reviewing).
- Shared word-data rules live in `src/game/wordData.ts`: the POS → entry-type codes (`posCode`/`posFilter`, e.g. adjective → `ADJ`), the headword dedupe (`dedupeByHeadword`), and the scrapper-row types + normalization (`RawWord`/`OxWord`/`normalizeRawWord`/`WordEntry` — runtime `Word` is an alias re-export of these via `types.ts`). Both translators, `wordPool.ts`, and `types.ts` import them — never copy the mapping, the dedupe loop, or the row shapes into another file.
